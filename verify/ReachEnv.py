...
from verify.divide_tool import str_to_list, max_min_clip, combine_bound_list, near_bound, \
...
temp = '((action + polemass_length * x[3] ** 2 * sin(x[2])) / total_mass)'
thetaacc = '((gravity * sin(x[2]) - cos(x[2]) * ' + temp + ') / (length * (4.0 / 3.0 - masspole * cos(x[2]) ** 2 / total_mass)))'
xacc = '(' + temp + ' - polemass_length * ' + thetaacc + ' * cos(x[2]) / total_mass)'


def x0_min_bound(current_bound):
    return current_bound[0] + current_bound[1] * timestep


def x0_max_bound(current_bound):
    return current_bound[4] + current_bound[5] * timestep


def x2_min_bound(current_bound):
    return current_bound[2] + current_bound[3] * timestep


def x2_max_bound(current_bound):
    return current_bound[6] + current_bound[7] * timestep
...
def calculate_reachable_sets(env, initial_bound, time_step, identifier=None):
    ...
    res_list = [[initial_bound[0], initial_bound[2], initial_bound[0 + actual_dim], initial_bound[2 + actual_dim]]]
    env.timer_reset()
    st = time.time()
    t = 0
    while True:
        ...
        for bound in bound_list:
            ...
            min_x2 = min(bound[2], min_x2)
            max_x2 = max(bound[2 + actual_dim], max_x2)
...
class ReachEnv():
    ...
    def get_next_bound_list(self, bound_list):
        ...
        # print(cnt)
        print('before aggregation:', cnt, end='    ')
        print('approximation:', self.time_op, 'aggregation:', self.time_agg)
        return u
    ...
    def gn(self, current):
        ...
        action, coefficients_tensor = self.network.fw_imp(s0)
        coefficients_array = coefficients_tensor.squeeze(0).detach().numpy()
        self.coefficients = coefficients_array

        offset = 0
        scala = 1
        action_str = str(self.coefficients[0])
        for i in range(half_dim):
            if self.coefficients[i] < 0:
                action_str = action_str + str(self.coefficients[i]) + '*' + 'x[' + str(i) + ']'
            else:
                action_str = action_str + '+' + str(self.coefficients[i]) + '*' + 'x[' + str(i) + ']'
    ...
    def execute_action(self, current):
        ...
        for i in range(half_dim):
            if i == 0:
                next_bounds[0] = np.clip(x0_min_bound(current), self.state_space[0][i], self.state_space[1][i])
                next_bounds[0 + half_dim] = np.clip(x0_max_bound(current), self.state_space[0][i],
                                                    self.state_space[1][i])
            elif i == 2:
                next_bounds[2] = np.clip(x2_min_bound(current), self.state_space[0][i], self.state_space[1][i])
                next_bounds[2 + half_dim] = np.clip(x2_max_bound(current), self.state_space[0][i],
                                                    self.state_space[1][i])
            else:
                fun_str = self.cd[i]
                fun_str = fun_str.replace('action', str(self.action))
