...
relu = True
# state_space = [[-1.5, -1.5, -2, -2.5], [1.5, 1.5, 2, 2.5]]
...
initial_intervals = [1, 1, 1, 1]
initial_intervals = [5, 5, 5, 20]
initial_intervals = [2.5, 2.5, 40, 40]
initial_intervals = [5, 5, 40, 40]
script_path = os.path.split(os.path.realpath(__file__))[0]
if relu:
    pt_file0 = os.path.join(script_path,
                            "tora_relu_abs-actor" + "_" + str(initial_intervals) + "_" + str(hidden_layer) + "_" + str(
                                hiden_size) + ".pt")
    pt_file1 = os.path.join(script_path,
                            "tora_relu_abs-critic" + "_" + str(initial_intervals) + "_" + str(hidden_layer) + "_" + str(
                                hiden_size) + ".pt")
    pt_file2 = os.path.join(script_path,
                            "tora_relu_abs-actor-target" + "_" + str(initial_intervals) + "_" + str(
                                hidden_layer) + "_" + str(hiden_size) + ".pt")
    pt_file3 = os.path.join(script_path,
                            "tora_relu_abs-critic-target" + "_" + str(initial_intervals) + "_" + str(
                                hidden_layer) + "_" + str(hiden_size) + ".pt")
else:
    pt_file0 = os.path.join(script_path,
...
def train_model(agent, max_epi=5000, terminate_pre=True):
    ...
    for j in range(10):
        ...
        for episode in range(max_epi):
            ...
            if terminate_pre and episode >= 100 and np.min(reward_list[-8:]) >= 970:
...
if __name__ == "__main__":
    ...
    # np.save('./tora_trace_list', arr=trace_list)

    hybrid = convert2hybrid('tora.json', state_space, initial_intervals, agent)
    hybrid.output_hybrid_str()

    # start_time = time.time()
    # repeat_list = reward_evaluate(agent, train_model, max_epi=200)
    # np.save('./tora_train_reward_pwl_' + str(initial_intervals), arr=repeat_list)
    # end_time = time.time()
    # print(end_time - start_time)  # 29
    ...
    # np.save('./tora_pwl_' + str(initial_intervals) + '_noisy_list' + str(noise_arr), arr=multiple_train_list)
    #
    # print('finished')
