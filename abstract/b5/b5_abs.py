...
if relu:
    pt_file0 = os.path.join(script_path,
                            "b5_relu_abs-actor" + "_" + str(initial_intervals) + "_" + str(hidden_layer) + "_" + str(
                                hiden_size) + ".pt")
    pt_file1 = os.path.join(script_path,
                            "b5_relu_abs-critic" + "_" + str(initial_intervals) + "_" + str(hidden_layer) + "_" + str(
                                hiden_size) + ".pt")
    pt_file2 = os.path.join(script_path, "b5_relu_abs-actor-target" + "_" + str(initial_intervals) + "_" + str(
        hidden_layer) + "_" + str(hiden_size) + ".pt")
    pt_file3 = os.path.join(script_path,
                            "b5_relu_abs-critic-target" + "_" + str(initial_intervals) + "_" + str(
                                hidden_layer) + "_" + str(hiden_size) + ".pt")
else:
    pt_file0 = os.path.join(script_path,
                            "b5_abs-actor" + "_" + str(initial_intervals) + "_" + str(hidden_layer) + "_" + str(
                                hiden_size) + ".pt")
    pt_file1 = os.path.join(script_path,
                            "b5_abs-critic" + "_" + str(initial_intervals) + "_" + str(hidden_layer) + "_" + str(
                                hiden_size) + ".pt")
    pt_file2 = os.path.join(script_path,
                            "b5_abs-actor-target" + "_" + str(initial_intervals) + "_" + str(hidden_layer) + "_" + str(
                                hiden_size) + ".pt")
    pt_file3 = os.path.join(script_path,
                            "b5_abs-critic-target" + "_" + str(initial_intervals) + "_" + str(hidden_layer) + "_" + str(
                                hiden_size) + ".pt")
...
def evaluate_trace(agent, episode=100):
    ...
    for l in range(episode):
        ...
        for step in range(10000):
            # env.render()
            a0 = agent.act(s0)
            s1, states, done = env.step_size(a0, step_size=0.001)
...
def train_model(agent, max_epi=3000, terminate_pre=True):
    ...
    for j in range(100):
        ...
        for episode in range(max_epi):
            ...
            if terminate_pre and episode >= 20 and np.min(reward_list[-7:]) >= 490:
...
if __name__ == "__main__":
    ...
    # np.save('./b5_trace_list', arr=trace_list)
    hybrid = convert2hybrid('b5.json', state_space, initial_intervals, agent)
    hybrid.output_hybrid_str()

    # start_time = time.time()
    # repeat_list = reward_evaluate(agent, train_model, max_epi=600)
    # np.save('./b5_train_reward_pwl_' + str(initial_intervals), arr=repeat_list)
    # end_time = time.time()
    # print(end_time - start_time)  # 136
    ...
    # np.save('./b5_pwl_' + str(initial_intervals) + '_noisy_list' + str(noise_arr), arr=multiple_train_list)
