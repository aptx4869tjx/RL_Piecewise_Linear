...
from abstract_env.b3 import B3Env
...
if relu:
    pt_file0 = os.path.join(script_path,
                            "b3_relu_abs-actor" + "_" + str(initial_intervals) + "_" + str(hidden_layer) + "_" + str(
                                hiden_size) + ".pt")
    pt_file1 = os.path.join(script_path,
                            "b3_relu_abs-critic" + "_" + str(initial_intervals) + "_" + str(hidden_layer) + "_" + str(
                                hiden_size) + ".pt")
    pt_file2 = os.path.join(script_path, "b3_relu_abs-actor-target" + "_" + str(initial_intervals) + "_" + str(
        hidden_layer) + "_" + str(hiden_size) + ".pt")
    pt_file3 = os.path.join(script_path,
                            "b3_relu_abs-critic-target" + "_" + str(initial_intervals) + "_" + str(
                                hidden_layer) + "_" + str(hiden_size) + ".pt")
else:
    pt_file0 = os.path.join(script_path,
                            "b3_abs-actor" + "_" + str(initial_intervals) + "_" + str(hidden_layer) + "_" + str(
                                hiden_size) + ".pt")
    pt_file1 = os.path.join(script_path,
                            "b3_abs-critic" + "_" + str(initial_intervals) + "_" + str(hidden_layer) + "_" + str(
                                hiden_size) + ".pt")
    pt_file2 = os.path.join(script_path,
                            "b3_abs-actor-target" + "_" + str(initial_intervals) + "_" + str(hidden_layer) + "_" + str(
                                hiden_size) + ".pt")
    pt_file3 = os.path.join(script_path,
                            "b3_abs-critic-target" + "_" + str(initial_intervals) + "_" + str(hidden_layer) + "_" + str(
                                hiden_size) + ".pt")
...
class Agent(object):
    def __init__(self, divide_tool):
        ...
        self.critic = Critic(s_dim + a_dim + dim, hiden_size, a_dim)
        self.critic_target = Critic(s_dim + a_dim + dim, hiden_size, a_dim)
    ...
    def update_egreed(self):
        self.e_greed = max(0.002, self.e_greed - 0.004)
...
def train_model(agent, max_epi=500, terminate_pre=True):
    ...
    for j in range(10):
        ...
        for episode in range(max_epi):
            ...
            if terminate_pre and episode >= 50 and np.min(reward_list[-8:]) >= 189:
...
if __name__ == "__main__":
    ...
    # np.save('./b3_trace_list', arr=trace_list)
    hybrid = convert2hybrid('b3.json', state_space, initial_intervals, agent)
    hybrid.output_hybrid_str()

    # start_time = time.time()
    # repeat_list = reward_evaluate(agent, train_model, max_epi=200)
    # np.save('./b3_train_reward_pwl_' + str(initial_intervals), arr=repeat_list)
    # end_time = time.time()
    # print(end_time - start_time)  # 58

    # noise_arr = [0.0005, 0.0005]
    # multiple_train_list = evaluate_multiple(agent, train_model, evaluate, env, state_space, noise_arr, step=50)
    # np.save('./b3_pwl_' + str(initial_intervals) + '_noisy_list' + str(noise_arr), arr=multiple_train_list)
