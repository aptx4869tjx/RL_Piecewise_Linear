...
if relu:
    pt_file0 = os.path.join(script_path, "tora-relu-actor" + str(hidden_layer) + "_" + str(hiden_size) + ".pt")
    pt_file1 = os.path.join(script_path, "tora-relu-critic" + str(hidden_layer) + "_" + str(hiden_size) + ".pt")
    pt_file2 = os.path.join(script_path, "tora-relu-actor-target" + str(hidden_layer) + "_" + str(hiden_size) + ".pt")
    pt_file3 = os.path.join(script_path, "tora-relu-critic-target" + str(hidden_layer) + "_" + str(hiden_size) + ".pt")
else:
    pt_file0 = os.path.join(script_path, "tora-actor" + str(hidden_layer) + "_" + str(hiden_size) + ".pt")
    pt_file1 = os.path.join(script_path, "tora-critic" + str(hidden_layer) + "_" + str(hiden_size) + ".pt")
    pt_file2 = os.path.join(script_path, "tora-actor-target" + str(hidden_layer) + "_" + str(hiden_size) + ".pt")
    pt_file3 = os.path.join(script_path, "tora-critic-target" + str(hidden_layer) + "_" + str(hiden_size) + ".pt")
...
class Agent(object):
    ...
    def update_egreed(self):
        self.e_greed = max(0.01, self.e_greed - 0.005)
...
def evaluate(agent):
    ...
    for l in range(100):
        ...
        for step in range(100):
            ...
            min_x1 = min(min_x1, s1[0])
            max_x1 = max(max_x1, s1[0])
            min_x2 = min(min_x2, s1[1])
            max_x2 = max(max_x2, s1[1])
            min_x3 = min(min_x3, s1[2])
            max_x3 = max(max_x3, s1[2])
            min_x4 = min(min_x4, s1[3])
            max_x4 = max(max_x4, s1[3])
            reward += r1
            s0 = s1
            if done:
                success += 1
                print('reach goal', s1, step, min_x1, max_x1, min_x2, max_x2, min_x3, max_x3, min_x4, max_x4)
                reach = True
                break
...
def train_model(agent, max_epi=5000, terminate_pre=True):
    ...
    for j in range(10):
        ...
        for episode in range(max_epi):
            ...
            if terminate_pre and episode >= 10 and np.min(reward_list[-8:]) >= 970:
...
if __name__ == "__main__":
    ...
    # np.save('./tora_Tanh_4_100_trace_list', arr=trace_list)

    start_time = time.time()
    repeat_list = reward_evaluate(agent, train_model, max_epi=200)
    np.save('./tora_train_reward', arr=repeat_list)
    end_time = time.time()  # 45
    print(end_time - start_time)

    # noise_arr = [0.0005, 0.0005, 0.0005, 0.0005]
    # state_space = [[-2.5, -2.5, -20, -20], [2.5, 2.5, 20, 20]]
    # multiple_train_list = evaluate_multiple(agent, train_model, evaluate, env, state_space, noise_arr, step=20)
    # np.save('./tora_noisy_list' + str(noise_arr), arr=multiple_train_list)
    print('finished')
